...
class AVLTreeList(object):
    ...
    """performs a left rotation on the edge from A to B, where B is A's right child, 
        and then a right rotation on the edge from B to C, where B is C's left child, and C is an AVL criminal.
        after the rotation, A and B have balance factor of 0, and C has a balance factor of 0 or -1.
    ...
    """performs a right rotation on the edge from A to B, where B is A's left child, 
            and then a left rotation on the edge from B to C, where B is C's right child, and C is an AVL criminal.
            after the rotation, A and B have balance factor of 0, and C has a balance factor of 0 or 1.
    ...
    def concat(self, lst):
        ...
        if lst.length() == 1:
            ...
            self.insert(self.length(), lst.root.value)
            return height
...
def fill_array_from_tree(node, values_list):
    ...
    if node.left.isRealNode():
        fill_array_from_tree(node.left, values_list)

    values_list.append(node.value)

    if node.right.isRealNode():
        fill_array_from_tree(node.right, values_list)
